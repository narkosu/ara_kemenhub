...
class LaporanController extends Controller
{
  ...
  public function actionLoadRekapitulasi($departement_id){
    $type   = (empty($_GET['typelaporan']) ? '' : $_GET['typelaporan']);
    $skjId  = (empty($_GET['skj']) ? '' : $_GET['skj']);
    ...
    $total_peserta = Peserta::model()->count('id_departemen = '.$departement_id);
    ...
    foreach ((array)$penilaian as $row){
      ...
      $return .= '<td>'.round((ceil($row->_count)/ceil($total_peserta))*100).'%</td>';
      $return .='</tr>';
      $total_penilaian += ceil($row->_count); 
    }
    $tidak_dinilai = $total_peserta - $total_penilaian;
    $return .='<tr>';
    $return .= '<td>BELUM DINILAI</td>';
    $return .= '<td>'. $tidak_dinilai .'</td>';
    $return .= '<td>'.round((ceil($tidak_dinilai)/ceil($total_peserta))*100).'%</td>';
    $return .='</tr>';
      
    $return .='<tr class="summary">';
      $return .= '<td>GRAND TOTAL</td>';
      $return .= '<td>'.ceil($total_peserta).'</td>';
      $return .= '<td>100%</td>';
      $return .='</tr>';
    echo $return;
	}
